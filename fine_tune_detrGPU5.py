...
class Detr(pl.LightningModule):
  def __init__(self, lr=1e-4, lr_backbone=1e-5, weight_decay=1e-4):
    ...
    config.id2label = {1: 'rudder'}
    config.label2id = {'rudder': 1}
...
def main():
    ...
    trainer = Trainer(gpus=4, gradient_clip_val=0.1, default_root_dir=os.path.join(root, 'ElodeaProject', name), max_epochs=10000, log_every_n_steps=56)
    trainer.fit(model)
#%%
